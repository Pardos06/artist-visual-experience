...
import type { Era, Album, Track, Milestone } from "../types/artist.types";
...
const formatDate = (date: string | Date | number | undefined): string => {
  if (date === undefined || date === null || date === "") return "";

  // Si es un número (ej. 2008), lo convertimos a string directamente
  if (typeof date === "number") {
    return String(date);
  }

  // Si es un string que representa únicamente dígitos (ej. "2008"), lo devolvemos tal cual
  if (typeof date === "string" && /^\d+$/.test(date)) {
    return date;
  }

  // Si es un string en formato YYYY-MM-DD (con o sin parte de hora/zona horaria)
  ...
  // Fallback: parsear como Date estándar, pero usando componentes UTC para evitar desfases locales
            ...
            <div className="era-intro">
              <div className="era-intro__header">
                ...
                <span className="era-years">
                  {era.startYear} - {era.endYear || "Presente"}
                </span>
              ...
              {era.milestones && era.milestones.length > 0 && (
                <div className="era-intro__milestones">
                  {era.milestones.map((moment: Milestone, i: number) => (
                    <div key={i} className="moment-item">
                      <span className="moment-year">{formatDate(moment.year)}</span>
              ...
              <div className="era-intro__album-grid">
                {era.albums?.map((album: Album) => (
                  ...
                  >
                    ...
                    <h3>{album.title}</h3>
                    {/* Cambio crítico: Se usa album.year */}
                    <small>{formatDate(album.year)}</small>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            >
              <div className="album-bento">
                ...
                <div className="album-bento__item album-bento__title">
                  ...
                  <span>{formatDate(selectedAlbum.year)}</span>
                </div>
